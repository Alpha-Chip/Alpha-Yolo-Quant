...
from yolov8n_quantisation.quantisation.utils.exponent import create_exponent_lookup_table, exponent_quant
...
def dfl_quant(layer_name, input, conv, scale_input=0, padding=1, stride=2):
    conv, conv_scale = quant_matrix(conv, K)
    conv_scale = np.transpose(conv_scale)
    bias_conv_scale = scale_input * conv_scale

    fake_bias = np.zeros(conv.shape).astype(np.int64)
    save_txt_weight(conv, fake_bias, layer_name, type='Conv2D', k=K, dir_names=dir_names)
    conv2d(layer_name, input, conv, fake_bias, dir_names, padding, stride)
    res = im2colzxc(input, conv, padding, stride)
    save_in_file(dir_names, conv, f'{layer_name}_conv.pickle')
    time.sleep(1.5)
    save_in_file(dir_names, fake_bias, f'{layer_name}_bias.pickle')
...
lookup_exp = create_exponent_lookup_table(14.8264799118042, 8)
...
def exponent(x, scale_x):
    ...
    res_silu = exponent_quant(x, lookup_exp)
    # res_silu *= res_conv_copy
    res_silu = np.round(res_silu)
...
def requant_last_layers(input_tensor, input_scale, layername, k=16):
    input_tensor, rescale, shift = requantize(input_tensor, input_scale, scale(14.8264799118042, k), k)  # max dfl 14.8264799118042   max_a_dict['dfl']
    result_scale = scale(14.8264799118042, k)
    save_txt_activations(input_tensor, f'{layername}_REQUANT', dir_names, type='act_conv', k=k, silu=True)
    save_txt_rescale_shift(input_tensor, rescale, shift, f'{layername}_REQUANT', dir_names, 'act_conv', k, silu=True)
...
x = np.clip(x, -np.int64(scale(1, k)), np.int64(scale(1, k)))
...
# -----------CONCAT_3X4----------
x_result_3, rescale, shift = requantize(x_result_3, scale_result_3, scale_result_4, k)
save_txt_activations(x_result_3, 'CONCAT_3X4_REQUANT', dir_names, type='act_silu', k=k, silu=True)
save_txt_rescale_shift(x_result_3, rescale, shift, 'CONCAT_3X4_REQUANT', dir_names, 'act_silu', k, silu=True)
...
# -----CONCAT_SPPFx3-----
x_result_3, rescale, shift = requantize(x_result_3, scale_result_3, scale_res_sppf_conv_1, k)
save_txt_activations(x_result_3, 'CONCAT_SPPFx3_REQUANT', dir_names, type='act_silu', k=k, silu=True)
save_txt_rescale_shift(x_result_3, rescale, shift, 'CONCAT_SPPFx3_REQUANT', dir_names, 'act_silu', k, silu=True)
...
x_result_5_up, scale_result_5_up = requant_last_layers(x_result_5_up, scale_result_5_up, 'x_result_5_up_2', 8)
...
# x_result_5_down, scale_result_5_down = requant_last_layers(x_result_5_down, scale_result_5_down, 'x_result_5_down_2')
...
x_result_6_up, scale_result_6_up = requant_last_layers(x_result_6_up, scale_result_6_up, 'x_result_6_up_2', 8)
...
# x_result_6_down, scale_result_6_down = requant_last_layers(x_result_6_down, scale_result_6_down, 'x_result_6_down_2')
...
x_up, scale_x_up = requant_last_layers(x_up, scale_x_up, 'x_up_2', 8)
...
# x_down, scale_x_down = requant_last_layers(x_down, scale_x_down, 'x_down_2')
...
# ---NEW---
def exponent_requant(el, scale_el, new_scale, layer_name):
    res_conv_copy = el.copy()
    res, rescale, shift = requantize(el, scale_el, new_scale, 16)
    save_txt_activations(res, layer_name, dir_names, type='act_conv', k=16)
    save_txt_rescale_shift(res, rescale, shift, layer_name, dir_names, 'act_conv', 16)
    # add_rescale_shift(layer_name, dir_names, res_conv_copy, rescale, shift)
    return res
...
x_result_5_down = exponent_requant(x_result_5_down, scale_result_5_down, sigmoid_scale, 'x_result_5_down_2')
x_result_6_down = exponent_requant(x_result_6_down, scale_result_6_down, sigmoid_scale, 'x_result_6_down_2')
x_down = exponent_requant(x_down, scale_x_down, sigmoid_scale, 'x_down_2')
...
# box, scale_box = softmax(box.reshape(b, 4, 16, a).transpose((0, 2, 1, 3)), scale_result_5_up, max_a_dict['dfl'], axis=1)

box = box.reshape(b, 4, 16, z).transpose((0, 2, 1, 3))

y = np.atleast_2d(box)
y = y - np.expand_dims(np.max(y, axis=1), 1)
y, scale_y = exponent(y, scale_result_5_up)
# y /= scale_y

ax_sum = np.expand_dims(np.sum(y, axis=1), 1)
p = np.int64(y / ax_sum * 127)
...
def quant_anchors(matrix, z):
    ...
    res_matrix = np.round(clip_matrix * all_scales)

    return res_matrix, all_scales


anchor, anchor_scale = quant_anchors(anchor, np.max(anchor))

dfl, scale_dfl = dfl_quant('dfl', p, weights['dfl.weight'],  127, padding=0, stride=1) # scale????
# dfl = np.float64(dfl) / scale_dfl
dfl = dfl.reshape(b, 4, z)
dfl, dfl_rescale, dfl_shift = requantize(dfl, scale_dfl, anchor_scale.item(), 16)
save_txt_activations(dfl, 'dfl', dir_names, type='act_conv', k=16)
save_txt_rescale_shift(dfl, dfl_rescale, dfl_shift, 'dfl', dir_names, 'act_conv', 16)
anchor = np.expand_dims(anchor, 0)
dbox = dist2bbox(dfl[0], anchor, xywh=True, dim=1) * strides
# print(dbox.shape)
# result_txt(dbox, 1)


cls = np.concatenate((x_result_5_down.reshape(1, 80, -1), x_result_6_down.reshape(1, 80, -1), x_down.reshape(1, 80, -1)), 2)
cls = sigmoid_quant(cls, lookup_final)
# cls /= 127
