...
import random
from yolov8n_quantisation.quantisation.utils.batch_norm import *
from yolov8n_quantisation.quantisation.stage_0 import D, W, R, ORIG_WEIGHTS, BATCHNF_WEIGHTS, MAIN_DIR_NAME, detect_1_channels
from collections import OrderedDict


ORIG_WEIGHTS = f'{MAIN_DIR_NAME}/results/{ORIG_WEIGHTS}'  # Путь до оригинальных весов
NEW_WEIGHTS = f'{MAIN_DIR_NAME}/results/{BATCHNF_WEIGHTS}'  # Путь до весов с BatchNormFusion
...
def coord(prediction):
    conf_thres = 0.25 # Threshold for classes
    nc = 80 # num classes
...
delete_batchn = []
conv_biases = []
weights = torch.load(ORIG_WEIGHTS)
for i, (key, value) in enumerate(weights.items()):
    if i <= 351:
        if list(weights.keys())[i].split('.')[-1] == 'weight' and list(weights.keys())[i+4].split('.')[-1] == 'running_var':
            delete_batchn.append(list(weights.keys())[i+1:i+6])
            weight = list(weights.values())[i:i+5][0]
            gamma = list(weights.values())[i:i+5][1]
            beta = list(weights.values())[i:i+5][2]
            mean = list(weights.values())[i:i+5][3]
            std = list(weights.values())[i:i+5][4]

            weight_fus, bias_fus = batchn_fusion(weight, gamma, beta, mean, std)
            weights[list(weights.keys())[i]] = weight_fus
            conv_bias = f"{'.'.join(list(weights.keys())[i].split('.')[:-1])}"
            conv_biases.append((conv_bias, bias_fus))

for el in delete_batchn:
    for key in el:
        del weights[key]

for el in conv_biases:
    if 'down' in el[0] or 'up' in el[0]:
        if int(el[0].split(".")[-1]) == 0:
            weights[f'{el[0]}.bias'] = el[1]
        else:
            weights[f'{el[0].split(".")[0]}.{int(el[0].split(".")[-1]) - 1}.bias'] = el[1]
    else:
        weights[f'{el[0]}.bias'] = el[1]


my_names = list(model.state_dict().keys())
full_dict = {}
c = 0
for i, (key, value) in enumerate(weights.items()):
    if 'bias' in my_names[i] and my_names[i].split('.')[1] != '4':
        for el in conv_biases:
            if my_names[i].split('.')[0] == el[0].split('.')[0] and (my_names[i].split('.')[1] == el[0].split('.')[1] or int(my_names[i].split('.')[1]) == int(el[0].split('.')[1])-1):
                full_dict[my_names[i]] = el[1]
                # print('ok', my_names[i])
    else:
        full_dict[my_names[i]] = list(weights.values())[c]
        c += 1
...
print('BATCHNF WEIGHTS SUCCESSFULLY SAVED!')
