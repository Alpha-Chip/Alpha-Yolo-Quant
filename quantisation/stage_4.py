...
from yolov8n_quantisation.quantisation.stage_0 import D, W, R, MAIN_DIR_NAME, BATCHNF_WEIGHTS, detect_1_channels
from yolov8n_quantisation.quantisation.utils.write_run_result import write_run_result
from yolov8n_quantisation.quantisation.utils.save_weights import save_batch
import deeplake
...
def coord(prediction):
    conf_thres = 0.00000001 # Threshold for classes 0.25
    nc = 80 # num classes
...
class Yolov8(nn.Module):
    ...
    def forward(self, x, ind):
        orig_img = x.clone()
        save_max_a(maxim_a, x, 'start')

        # -----Conv_P1-----
        x = self.conv0[0](x)  # Conv
        save_max_a(maxim_a, x, 'conv_p1')
        # matrix_txt_name(x, 'CONV_P1', 'matrix_batchf.txt')
        x = self.conv0[1](x)  # SiLU

        # matrix_txt_name(x, 'SiLU_P1', 'matrix_batchf.txt')
        # save_max_a(maxim_a, x, 'silu_p1')

        # -----Conv_P2-----
        x = self.conv1[0](x)
        save_max_a(maxim_a, x, 'conv_p2')
        save_batch(MAIN_DIR_NAME, x, 'conv_p2', f'b_{ind}.pickle')
        # matrix_txt_name(x, 'CONV_P2', 'matrix_batchf.txt')
        x = self.conv1[1](x)
        # matrix_txt_name(x, 'SiLU_P2', 'matrix_batchf.txt')

        # -----C2F_2-----
        x = self.cf2_conv_0[0](x)
        save_max_a(maxim_a, x, 'conv_0_c2f')
        save_batch(MAIN_DIR_NAME, x, 'conv_0_c2f', f'b_{ind}.pickle')
        # matrix_txt_name(x, 'conv_0_c2f', 'matrix_batchf.txt')
        x = self.cf2_conv_0[1](x)
        # matrix_txt_name(x, 'silu_0_c2f', 'matrix_batchf.txt')
        ...
        save_max_a(maxim_a, x, 'conv_b_0_c2f')
        save_batch(MAIN_DIR_NAME, x, 'conv_b_0_c2f', f'b_{ind}.pickle')
        x = self.cf2_bottle_0[1](x)
        # matrix_txt_name(x, 'silu_0_c2f', 'matrix_batchf.txt')
        x = self.cf2_bottle_0[2](x)
        save_max_a(maxim_a, x, 'conv_b_1_c2f')
        save_batch(MAIN_DIR_NAME, x, 'conv_b_1_c2f', f'b_{ind}.pickle')
        x = self.cf2_bottle_0[3](x)
        # matrix_txt_name(x, 'silu_0_c2f', 'matrix_batchf.txt')
        # save_max_a(maxim_a, x, 'conv_b_0_c2f')
        x += x_bottle_0
        # matrix_txt_name(x, 'plus', 'matrix_batchf.txt')
        # --Bottleneck--
        x = torch.cat((x1, x2, x), dim=1)
        x = self.cf2_conv_1[0](x)
        save_max_a(maxim_a, x, 'conv_b_2_c2f')
        save_batch(MAIN_DIR_NAME, x, 'conv_b_2_c2f', f'b_{ind}.pickle')
        x = self.cf2_conv_1[1](x)
        # matrix_txt_name(x, '0_c2f', 'matrix_batchf.txt')

        # -----Conv_P3-----
        x = self.conv3[0](x)
        save_max_a(maxim_a, x, 'conv_p3')
        save_batch(MAIN_DIR_NAME, x, 'conv_p3', f'b_{ind}.pickle')
        x = self.conv3[1](x)
        # matrix_txt_name(x, 'conv_3', 'matrix_batchf.txt')

        # -----C2F_4-----
        x = self.cf2_conv_2[0](x)
        save_max_a(maxim_a, x, 'conv_2_c2f')
        save_batch(MAIN_DIR_NAME, x, 'conv_2_c2f', f'b_{ind}.pickle')
        x = self.cf2_conv_2[1](x)
        # matrix_txt_name(x, 'conv_2', 'matrix_batchf.txt')
        x1, x = split(x, x.shape[1])
        x2 = x.clone()
        # --Bottleneck--
        x_bottle_0 = x.clone()
        x = self.cf2_bottle_2[0](x)
        save_max_a(maxim_a, x, 'conv_b1_c2f')
        save_batch(MAIN_DIR_NAME, x, 'conv_b1_c2f', f'b_{ind}.pickle')
        x = self.cf2_bottle_2[1](x)
        x = self.cf2_bottle_2[2](x)
        save_max_a(maxim_a, x, 'conv_b2_c2f')
        save_batch(MAIN_DIR_NAME, x, 'conv_b2_c2f', f'b_{ind}.pickle')
        x = self.cf2_bottle_2[3](x)
        x += x_bottle_0
        # matrix_txt_name(x, 'conv_1_c2f', 'matrix_batchf.txt')
        x3 = x.clone()
        x_bottle_1 = x.clone()
        x = self.cf2_bottle_3[0](x)
        save_max_a(maxim_a, x, 'conv_b3_c2f')
        save_batch(MAIN_DIR_NAME, x, 'conv_b3_c2f', f'b_{ind}.pickle')
        x = self.cf2_bottle_3[1](x)
        x = self.cf2_bottle_3[2](x)
        save_max_a(maxim_a, x, 'conv_b4_c2f')
        save_batch(MAIN_DIR_NAME, x, 'conv_b4_c2f', f'b_{ind}.pickle')
        x = self.cf2_bottle_3[3](x)
        x += x_bottle_1
        # matrix_txt_name(x, 'conv_2zxc_c2f', 'matrix_batchf.txt')
        # --Bottleneck--
        x = torch.cat((x1, x2, x3, x), dim=1)
        x = self.cf2_conv_3[0](x)
        save_max_a(maxim_a, x, 'conv_b5_c2f')
        save_batch(MAIN_DIR_NAME, x, 'conv_b5_c2f', f'b_{ind}.pickle')
        x = self.cf2_conv_3[1](x)
        # matrix_txt_name(x, 'conv_2last_c2f', 'matrix_batchf.txt')
        x_result_1 = x.clone()  # ------------------------------------->

        # -----Conv_P4-----
        x = self.conv5[0](x)
        save_max_a(maxim_a, x, 'conv_5')
        save_batch(MAIN_DIR_NAME, x, 'conv_5', f'b_{ind}.pickle')
        x = self.conv5[1](x)
        # matrix_txt_name(x, 'conv_2last_c2f', 'matrix_batchf.txt')

        # -----C2F_6-----
        x = self.cf2_conv_4[0](x)
        save_max_a(maxim_a, x, 'cf2_conv_4')
        save_batch(MAIN_DIR_NAME, x, 'cf2_conv_4', f'b_{ind}.pickle')
        x = self.cf2_conv_4[1](x)
        # matrix_txt_name(x, 'cf2_conv_4', 'matrix_batchf.txt')
        x1, x = split(x, x.shape[1])
        x2 = x.clone()
        # --Bottleneck--
        x_bottle_0 = x.clone()
        x = self.cf2_bottle_4[0](x)
        save_max_a(maxim_a, x, 'cf2_bconv_4')
        save_batch(MAIN_DIR_NAME, x, 'cf2_bconv_4', f'b_{ind}.pickle')
        x = self.cf2_bottle_4[1](x)
        x = self.cf2_bottle_4[2](x)
        save_max_a(maxim_a, x, 'cf2_bconv1_4')
        save_batch(MAIN_DIR_NAME, x, 'cf2_bconv1_4', f'b_{ind}.pickle')
        x = self.cf2_bottle_4[3](x)
        x += x_bottle_0
        x3 = x.clone()
        x_bottle_1 = x.clone()
        x = self.cf2_bottle_5[0](x)
        save_max_a(maxim_a, x, 'cf2_bconv_5')
        save_batch(MAIN_DIR_NAME, x, 'cf2_bconv_5', f'b_{ind}.pickle')
        x = self.cf2_bottle_5[1](x)
        x = self.cf2_bottle_5[2](x)
        save_max_a(maxim_a, x, 'cf2_bconv1_5')
        save_batch(MAIN_DIR_NAME, x, 'cf2_bconv1_5', f'b_{ind}.pickle')
        x = self.cf2_bottle_5[3](x)
        x += x_bottle_1
        # --Bottleneck--
        x = torch.cat((x1, x2, x3, x), dim=1)
        x = self.cf2_conv_5[0](x)
        save_max_a(maxim_a, x, 'cf2_6_conv_last')
        save_batch(MAIN_DIR_NAME, x, 'cf2_6_conv_last', f'b_{ind}.pickle')
        ...
        # -----Conv_P5-----
        x = self.conv7[0](x)
        save_max_a(maxim_a, x, 'conv7')
        save_batch(MAIN_DIR_NAME, x, 'conv7', f'b_{ind}.pickle')
        x = self.conv7[1](x)
        # matrix_txt_name(x, 'conv7', 'matrix_batchf.txt')

        # -----C2F_8-----
        x = self.cf2_conv_6[0](x)
        save_max_a(maxim_a, x, 'cf2_conv_6')
        save_batch(MAIN_DIR_NAME, x, 'cf2_conv_6', f'b_{ind}.pickle')
        ...
        save_max_a(maxim_a, x, 'cf2_bottle_6')
        save_batch(MAIN_DIR_NAME, x, 'cf2_bottle_6', f'b_{ind}.pickle')
        x = self.cf2_bottle_6[1](x)
        x = self.cf2_bottle_6[2](x)
        save_max_a(maxim_a, x, 'cf2_bottle_61')
        save_batch(MAIN_DIR_NAME, x, 'cf2_bottle_61', f'b_{ind}.pickle')
        x = self.cf2_bottle_6[3](x)
        x += x_bottle_0
        # --Bottleneck--
        x = torch.cat((x1, x2, x), dim=1)
        x = self.cf2_conv_7[0](x)
        save_max_a(maxim_a, x, 'cf2_conv_7')
        save_batch(MAIN_DIR_NAME, x, 'cf2_conv_7', f'b_{ind}.pickle')
        x = self.cf2_conv_7[1](x)
        # matrix_txt_name(x, 'cf2_conv_7', 'matrix_batchf.txt')

        # -----SPPF-----
        x = self.sppf_conv_1[0](x)
        save_max_a(maxim_a, x, 'sppf_conv_1')
        save_batch(MAIN_DIR_NAME, x, 'sppf_conv_1', f'b_{ind}.pickle')
        x = self.sppf_conv_1[1](x)
        # matrix_txt_name(x, 'sppf_conv_1', 'matrix_batchf.txt')
        x1 = x.clone()
        x = self.maxp_0(x)
        # matrix_txt_name(x, 'maxp_0', 'matrix_batchf.txt')
        x2 = x.clone()
        x = self.maxp_1(x)
        x3 = x.clone()
        x = self.maxp_2(x)
        ...
        save_max_a(maxim_a, x, 'sppf_conv_2')
        save_batch(MAIN_DIR_NAME, x, 'sppf_conv_2', f'b_{ind}.pickle')
        x = self.sppf_conv_2[1](x)  # ------------------------------------->
        # matrix_txt_name(x, 'sppf_conv_2', 'matrix_batchf.txt')
        x_result_3 = x.clone()  # ------------------------------------->

        # -----UPSAMPLE_10-----
        x_result_3 = self.ups_10(x_result_3)
        # matrix_txt_name(x_result_3, 'ups_10', 'matrix_batchf.txt')

        # -----CONCAT_2X3-----
        x_result_3 = torch.cat((x_result_3, x_result_2), dim=1)
        # matrix_txt_name(x_result_3, 'CONCAT_2X3', 'matrix_batchf.txt')

        # -----C2F_12-----
        x_result_3 = self.cf2_conv_8[0](x_result_3)
        save_max_a(maxim_a, x_result_3, 'cf2_conv_8')
        save_batch(MAIN_DIR_NAME, x, 'cf2_conv_8', f'b_{ind}.pickle')
        x_result_3 = self.cf2_conv_8[1](x_result_3)
        # matrix_txt_name(x_result_3, 'x_result_3', 'matrix_batchf.txt')
        x1, x_result_3 = split(x_result_3, x_result_3.shape[1])
        x2 = x_result_3.clone()
        # matrix_txt_name(x1, 'x1', 'matrix_batchf.txt')
        # --Bottleneck--
        x_result_3 = self.cf2_bottle_7[0](x_result_3)
        save_max_a(maxim_a, x_result_3, 'cf2_conv_80')
        save_batch(MAIN_DIR_NAME, x, 'cf2_conv_80', f'b_{ind}.pickle')
        x_result_3 = self.cf2_bottle_7[1](x_result_3)
        x_result_3 = self.cf2_bottle_7[2](x_result_3)
        save_max_a(maxim_a, x_result_3, 'cf2_conv_81')
        save_batch(MAIN_DIR_NAME, x, 'cf2_conv_81', f'b_{ind}.pickle')
        ...
        save_max_a(maxim_a, x_result_3, 'cf2_conv_9')
        save_batch(MAIN_DIR_NAME, x, 'cf2_conv_9', f'b_{ind}.pickle')
        x_result_3 = self.cf2_conv_9[1](x_result_3)
        x_result_4 = x_result_3.clone()  # ------------------------------------->
        # matrix_txt_name(x_result_4, 'C2F_12', 'matrix_batchf.txt')

        # -----UPSAMPLE_13-----
        x_result_3 = self.ups_13(x_result_3)
        # matrix_txt_name(x_result_3, 'UPSAMPLE_13', 'matrix_batchf.txt')

        # -----CONCAT_1X3-----
        x_result_3 = torch.cat((x_result_3, x_result_1), dim=1)
        # matrix_txt_name(x_result_3, 'CONCAT_1X3', 'matrix_batchf.txt')

        # -----C2F_15-----
        x_result_3 = self.cf2_conv_10[0](x_result_3)
        save_max_a(maxim_a, x_result_3, 'cf2_conv_10')
        save_batch(MAIN_DIR_NAME, x, 'cf2_conv_10', f'b_{ind}.pickle')
        x_result_3 = self.cf2_conv_10[1](x_result_3)
        x1, x_result_3 = split(x_result_3, x_result_3.shape[1])
        x2 = x_result_3.clone()
        # --Bottleneck--
        x_result_3 = self.cf2_bottle_8[0](x_result_3)
        save_max_a(maxim_a, x_result_3, 'cf2_bottle_8')
        save_batch(MAIN_DIR_NAME, x, 'cf2_bottle_8', f'b_{ind}.pickle')
        x_result_3 = self.cf2_bottle_8[1](x_result_3)
        x_result_3 = self.cf2_bottle_8[2](x_result_3)
        save_max_a(maxim_a, x_result_3, 'cf2_bottle_81')
        save_batch(MAIN_DIR_NAME, x, 'cf2_bottle_81', f'b_{ind}.pickle')
        x_result_3 = self.cf2_bottle_8[3](x_result_3)
        # --Bottleneck--
        x_result_3 = torch.cat((x1, x2, x_result_3), dim=1)
        x_result_3 = self.cf2_conv_11[0](x_result_3)
        save_max_a(maxim_a, x_result_3, 'cf2_conv_11')
        save_batch(MAIN_DIR_NAME, x, 'cf2_conv_11', f'b_{ind}.pickle')
        x_result_3 = self.cf2_conv_11[1](x_result_3)
        x_result_5 = x_result_3.clone()  # ------------------------------------->
        # matrix_txt_name(x_result_5, 'C2F_15', 'matrix_batchf.txt')

        # -----Conv_P3-----
        x_result_3 = self.conv8[0](x_result_3)
        save_max_a(maxim_a, x_result_3, 'conv8')
        save_batch(MAIN_DIR_NAME, x, 'conv8', f'b_{ind}.pickle')
        x_result_3 = self.conv8[1](x_result_3)
        # matrix_txt_name(x_result_3, 'conv8', 'matrix_batchf.txt')

        # -----CONCAT_3X4-----
        x_result_3 = torch.cat((x_result_3, x_result_4), dim=1)
        # print(x_result_3.shape, 'zxc')
        # matrix_txt_name(x_result_3, 'cat', 'matrix_batchf.txt')

        # -----C2F_18-----
        x_result_3 = self.cf2_conv_12[0](x_result_3)
        save_max_a(maxim_a, x_result_3, 'cf2_conv_12')
        save_batch(MAIN_DIR_NAME, x, 'cf2_conv_12', f'b_{ind}.pickle')
        x_result_3 = self.cf2_conv_12[1](x_result_3)
        x1, x_result_3 = split(x_result_3, x_result_3.shape[1])
        x2 = x_result_3.clone()
        # --Bottleneck--
        x_result_3 = self.cf2_bottle_9[0](x_result_3)
        save_max_a(maxim_a, x_result_3, 'cf2_bottle_9')
        save_batch(MAIN_DIR_NAME, x, 'cf2_bottle_9', f'b_{ind}.pickle')
        x_result_3 = self.cf2_bottle_9[1](x_result_3)
        x_result_3 = self.cf2_bottle_9[2](x_result_3)
        save_max_a(maxim_a, x_result_3, 'cf2_bottle_90')
        save_batch(MAIN_DIR_NAME, x, 'cf2_bottle_90', f'b_{ind}.pickle')
        x_result_3 = self.cf2_bottle_9[3](x_result_3)
        # --Bottleneck--
        x_result_3 = torch.cat((x1, x2, x_result_3), dim=1)
        x_result_3 = self.cf2_conv_13[0](x_result_3)
        save_max_a(maxim_a, x_result_3, 'cf2_conv_13')
        save_batch(MAIN_DIR_NAME, x, 'cf2_conv_13', f'b_{ind}.pickle')
        ...
        save_max_a(maxim_a, x_result_3, 'conv9')
        save_batch(MAIN_DIR_NAME, x, 'conv9', f'b_{ind}.pickle')
        x_result_3 = self.conv9[1](x_result_3)
        # matrix_txt_name(x_result_3, 'Conv_19', 'matrix_batchf.txt')

        # -----CONCAT_Xx3-----
        x = torch.cat((x_result_3, x), dim=1)
        # matrix_txt_name(x, 'CONCAT_Xx3', 'matrix_batchf.txt')

        # -----C2F_21-----
        x = self.cf2_conv_14[0](x)
        save_max_a(maxim_a, x, 'cf2_conv_14')
        save_batch(MAIN_DIR_NAME, x, 'cf2_conv_14', f'b_{ind}.pickle')
        x = self.cf2_conv_14[1](x)
        x1, x = split(x, x.shape[1])
        x2 = x.clone()
        # --Bottleneck--
        x = self.cf2_bottle_10[0](x)
        save_max_a(maxim_a, x, 'cf2_bottle_10')
        save_batch(MAIN_DIR_NAME, x, 'cf2_bottle_10', f'b_{ind}.pickle')
        x = self.cf2_bottle_10[1](x)
        x = self.cf2_bottle_10[2](x)
        save_max_a(maxim_a, x, 'cf2_bottle_101')
        save_batch(MAIN_DIR_NAME, x, 'cf2_bottle_101', f'b_{ind}.pickle')
        x = self.cf2_bottle_10[3](x)
        # --Bottleneck--
        x = torch.cat((x1, x2, x), dim=1)
        x = self.cf2_conv_15[0](x)
        save_max_a(maxim_a, x, 'cf2_conv_15')
        save_batch(MAIN_DIR_NAME, x, 'cf2_conv_15', f'b_{ind}.pickle')
        x = self.cf2_conv_15[1](x)
        # matrix_txt_name(x, 'C2F_21', 'matrix_batchf.txt')

        # -----Detect_5-----
        x_result_5_up, x_result_5_down = up_down_parsing(x_result_5)
        # matrix_txt_name(x_result_5_down, 'x_result_5_up', 'matrix_batchf.txt')
        x_result_5_up = self.detect_5_up[0](x_result_5_up)
        save_max_a(maxim_a, x_result_5_up, 'x_result_5_up_0')
        save_batch(MAIN_DIR_NAME, x, 'x_result_5_up_0', f'b_{ind}.pickle')
        x_result_5_up = self.detect_5_up[1](x_result_5_up)
        # matrix_txt_name(x_result_5_up, 'x_result_5_up_0', 'matrix_batchf.txt')
        x_result_5_up = self.detect_5_up[2](x_result_5_up)
        save_max_a(maxim_a, x_result_5_up, 'x_result_5_up_1')
        save_batch(MAIN_DIR_NAME, x, 'x_result_5_up_1', f'b_{ind}.pickle')
        x_result_5_up = self.detect_5_up[3](x_result_5_up)
        # matrix_txt_name(x_result_5_up, 'x_result_5_up_1', 'matrix_batchf.txt')
        x_result_5_up = self.detect_5_up[4](x_result_5_up)
        save_max_a(maxim_a, x_result_5_up, 'x_result_5_up_2')
        save_batch(MAIN_DIR_NAME, x, 'x_result_5_up_2', f'b_{ind}.pickle')
        # matrix_txt_name(x_result_5_up, 'x_result_5_up_2', 'matrix_batchf.txt')

        x_result_5_down = self.detect_5_down[0](x_result_5_down)
        save_max_a(maxim_a, x_result_5_down, 'x_result_5_down_0')
        save_batch(MAIN_DIR_NAME, x, 'x_result_5_down_0', f'b_{ind}.pickle')
        x_result_5_down = self.detect_5_down[1](x_result_5_down)
        # matrix_txt_name(x_result_5_down, 'x_result_5_down_0', 'matrix_batchf.txt')
        x_result_5_down = self.detect_5_down[2](x_result_5_down)
        save_max_a(maxim_a, x_result_5_down, 'x_result_5_down_1')
        save_batch(MAIN_DIR_NAME, x, 'x_result_5_down_1', f'b_{ind}.pickle')
        x_result_5_down = self.detect_5_down[3](x_result_5_down)
        # matrix_txt_name(x_result_5_down, 'x_result_5_down_2', 'matrix_batchf.txt')
        x_result_5_down = self.detect_5_down[4](x_result_5_down)
        save_max_a(maxim_a, x_result_5_down, 'x_result_5_down_2')
        save_batch(MAIN_DIR_NAME, x, 'x_result_5_down_2', f'b_{ind}.pickle')
        # matrix_txt_name(x_result_5_down, 'x_result_5_down_4', 'matrix_batchf.txt')

        # -----Detect_6-----
        x_result_6_up, x_result_6_down = up_down_parsing(x_result_6)
        x_result_6_up = self.detect_6_up[0](x_result_6_up)
        save_max_a(maxim_a, x_result_6_up, 'x_result_6_up_0')
        save_batch(MAIN_DIR_NAME, x, 'x_result_6_up_0', f'b_{ind}.pickle')
        x_result_6_up = self.detect_6_up[1](x_result_6_up)
        # matrix_txt_name(x_result_6_up, 'x_result_6_up_0', 'matrix_batchf.txt')
        x_result_6_up = self.detect_6_up[2](x_result_6_up)
        save_max_a(maxim_a, x_result_6_up, 'x_result_6_up_1')
        save_batch(MAIN_DIR_NAME, x, 'x_result_6_up_1', f'b_{ind}.pickle')
        x_result_6_up = self.detect_6_up[3](x_result_6_up)
        # matrix_txt_name(x_result_6_up, 'x_result_6_up_2', 'matrix_batchf.txt')
        x_result_6_up = self.detect_6_up[4](x_result_6_up)
        save_max_a(maxim_a, x_result_6_up, 'x_result_6_up_2')
        save_batch(MAIN_DIR_NAME, x, 'x_result_6_up_2', f'b_{ind}.pickle')
        # matrix_txt_name(x_result_6_up, 'x_result_6_up_4', 'matrix_batchf.txt')
        x_result_6_down = self.detect_6_down[0](x_result_6_down)
        save_max_a(maxim_a, x_result_6_down, 'x_result_6_down_0')
        save_batch(MAIN_DIR_NAME, x, 'x_result_6_down_0', f'b_{ind}.pickle')
        x_result_6_down = self.detect_6_down[1](x_result_6_down)
        # matrix_txt_name(x_result_6_down, 'x_result_6_down_0', 'matrix_batchf.txt')
        x_result_6_down = self.detect_6_down[2](x_result_6_down)
        save_max_a(maxim_a, x_result_6_down, 'x_result_6_down_1')
        save_batch(MAIN_DIR_NAME, x, 'x_result_6_down_1', f'b_{ind}.pickle')
        x_result_6_down = self.detect_6_down[3](x_result_6_down)
        # matrix_txt_name(x_result_6_down, 'x_result_6_down_2', 'matrix_batchf.txt')
        x_result_6_down = self.detect_6_down[4](x_result_6_down)
        save_max_a(maxim_a, x_result_6_down, 'x_result_6_down_2')
        save_batch(MAIN_DIR_NAME, x, 'x_result_6_down_2', f'b_{ind}.pickle')
        # matrix_txt_name(x_result_6_down, 'x_result_6_down_4', 'matrix_batchf.txt')

        # -----Detect_x-----
        x_up, x_down = up_down_parsing(x)
        x_up = self.detect_x_up[0](x_up)
        save_max_a(maxim_a, x_up, 'x_up_0')
        save_batch(MAIN_DIR_NAME, x, 'x_up_0', f'b_{ind}.pickle')
        x_up = self.detect_x_up[1](x_up)
        # matrix_txt_name(x_up, 'x_up_0', 'matrix_batchf.txt')
        x_up = self.detect_x_up[2](x_up)
        save_max_a(maxim_a, x_up, 'x_up_1')
        save_batch(MAIN_DIR_NAME, x, 'x_up_1', f'b_{ind}.pickle')
        x_up = self.detect_x_up[3](x_up)
        # matrix_txt_name(x_up, 'x_up_2', 'matrix_batchf.txt')
        x_up = self.detect_x_up[4](x_up)
        save_max_a(maxim_a, x_up, 'x_up_2')
        save_batch(MAIN_DIR_NAME, x, 'x_up_2', f'b_{ind}.pickle')
        # matrix_txt_name(x_up, 'x_up_4', 'matrix_batchf.txt')
        x_down = self.detect_x_down[0](x_down)
        save_max_a(maxim_a, x_down, 'x_down_0')
        save_batch(MAIN_DIR_NAME, x, 'x_down_0', f'b_{ind}.pickle')
        x_down = self.detect_x_down[1](x_down)
        # matrix_txt_name(x_down, 'x_down_0', 'matrix_batchf.txt')
        x_down = self.detect_x_down[2](x_down)
        save_max_a(maxim_a, x_down, 'x_down_1')
        save_batch(MAIN_DIR_NAME, x, 'x_down_1', f'b_{ind}.pickle')
        x_down = self.detect_x_down[3](x_down)
        # matrix_txt_name(x_down, 'x_down_2', 'matrix_batchf.txt')
        x_down = self.detect_x_down[4](x_down)
        save_max_a(maxim_a, x_down, 'x_down_2')
        save_batch(MAIN_DIR_NAME, x, 'x_down_2', f'b_{ind}.pickle')
        # matrix_txt_name(x_down, 'x_down_4', 'matrix_batchf.txt')
        # print(x_result_5_up.shape, x_result_6_up.shape, x_up.shape)
        ...
        cls = cls.sigmoid()
        # print(cls.shape)
        # matrix_txt_flatten_name(cls, 'cls', 'matrix_batchf.txt')

        dbox_cls = torch.cat((dbox, cls), 1)
        # print(dbox_cls.shape)
        # matrix_txt_flatten_name(dbox_cls, 'dbox_cls', 'matrix_batchf.txt')
...
model.load_state_dict(torch.load(f'{MAIN_DIR_NAME}/results/{BATCHNF_WEIGHTS}'))
val_dataset = deeplake.load("hub://activeloop/coco-val")
# val_dataset = deeplake.load("hub://activeloop/coco-train")
print(val_dataset)
...
with torch.no_grad():
    for ind, batch in enumerate(tqdm(trainloader)):
        if ind == 5000:
            break
        # img = scale_img(batch['images'])
        # img = img.to(device)
        img = batch_transform(batch['images'])
        img = img.to(device)
        # # print(target[0]['bbox'])
        boxes, classes = model(img, ind)
        # ann = map_from_torch(ann, ind, boxes, classes)
        ...
        # # plot_res_box_classes(img, batch['boxes'], batch['categories'])

# ann.to_csv(f'ann_orig_BNF.csv', index=False)
det.to_csv(f'{MAIN_DIR_NAME}/results/det_orig_BNF.csv', index=False)
...
# write_run_result(sum(result_ap) / len(result_ap), 4)
write_run_result(result_ap, 4)


with open(f'{MAIN_DIR_NAME}/results/max_a_all.txt', 'w') as f_obj:
# with open(f'{MAIN_DIR_NAME}/results/max_a.txt', 'w') as f_obj:
    for key, value in maxim_a.items():
        # f_obj.write(f"{key}: {max(value)}\n")
