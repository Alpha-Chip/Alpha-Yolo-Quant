...
def quant_matrix(matrix, k, start=False):
    ...
    # all_scales = np.zeros((matrix.shape[0], matrix.shape[1]))
    # for batch in range(matrix.shape[0]):
    #     for channel in range(matrix.shape[1]):
    #         a_max_channel = a(matrix[batch, channel, :, :])
    #         clip_batch_matrix = new_clip(matrix[batch, channel], a_max_channel)
    #         scale_input_channel = scale(a_max_channel, k)
    #         all_scales[batch, channel] += scale_input_channel
    #         res_matrix[batch, channel] = np.int64(np.round(clip_batch_matrix * scale_input_channel))
    # return np.int64(res_matrix), all_scales
    res_matrix = np.zeros(matrix.shape)
    all_scales = np.zeros((matrix.shape[0], 1))
    for batch in range(matrix.shape[0]):
        if start == True:
            a_max_channel = 1
        else:
        ...
        res_matrix[batch] = np.int64(np.round(clip_batch_matrix * scale_input_channel))
    return np.int64(res_matrix), all_scales
