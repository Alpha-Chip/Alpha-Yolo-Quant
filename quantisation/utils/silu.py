...
def create_sigmoid_lookup_table(max_conv_value, bit_size_act):
    global SIGMOID_LOOKUP

    # First create lookup table
    max_val = 2 ** (bit_size_act - 1) - 1
    SIGMOID_LOOKUP = dict()
    for i in range(-max_val, max_val + 1, 1):
        arr = np.array((i,))
        d = dequantize(arr, max_conv_value, bit_size_act)[0]
        # d = dequantize(arr, 6, bit_size_act)[0]
        float_val = np.array((sigmoid(d),))
        quant_val = quantize(float_val, 1, bit_size_act)[0]
        SIGMOID_LOOKUP[i] = quant_val
        # print(arr, float_val, quant_val)
    with open(f'utils/sigmoid_table_{bit_size_act}_bit.txt', 'w') as f_obj:
        f_obj.write(f'// SIGMOID TABLE FOR {bit_size_act} BIT\n\n')
        for key, value in SIGMOID_LOOKUP.items():
            f_obj.write(f'{key} = {value}\n')
    return SIGMOID_LOOKUP


# print(create_sigmoid_lookup_table(10))


def sigmoid_quant(x, lookup):
    # global SIGMOID_LOOKUP
